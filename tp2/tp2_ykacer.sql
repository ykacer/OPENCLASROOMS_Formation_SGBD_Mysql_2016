...
SELECT DATE_FORMAT(Article.date_publication,'%d/%m/%Y') AS date_publication ,Utilisateur.pseudo AS auteur, Article.titre AS titre,
SUBSTRING(Article.resume,1,20) AS resume, COUNT(Commentaire.article_id) AS nb_commentaires -- on raccourcit les resume aux 20 premiers caractères pour une meilleure lisibilité du résultat de requete
FROM Article 
...
SELECT DATE_FORMAT(Article.date_publication,'%d %M ''%y') AS date_publication,Utilisateur.pseudo AS auteur, Article.titre AS titre,
SUBSTRING(Article.resume,1,20) AS resume
FROM Article
INNER JOIN Utilisateur ON Article.auteur_id = Utilisateur.id
...
SELECT DATE_FORMAT(Article.date_publication,'%d/%m/%Y - %H:%i') AS date_publication,Utilisateur.pseudo AS auteur, Article.titre AS titre,
SUBSTRING(Article.resume,1,20) AS resume
FROM Article
INNER JOIN Utilisateur ON Article.auteur_id = Utilisateur.id
INNER JOIN Categorie_article ON Article.id = Categorie_article.article_id
WHERE Categorie_article.categorie_id = 3
GROUP BY Article.date_publication DESC;

-- ********** PAGE ARTICLE **********

SELECT Commentaire.contenu,
DATE_FORMAT(Commentaire.date_commentaire, '%d/%m/%Y') AS date_publication, Utilisateur.pseudo AS pseudo
FROM Commentaire
...
SELECT Article.id AS id,DATE_FORMAT(Article.date_publication,'%d %M %Y à %H heures %i') AS date_publication,
Utilisateur.pseudo AS pseudo, GROUP_CONCAT(Categorie.nom SEPARATOR ' - ') AS categories
FROM Article
INNER JOIN Utilisateur ON Article.auteur_id = Utilisateur.id
INNER JOIN Categorie_article ON Article.id = Categorie_article.article_id
